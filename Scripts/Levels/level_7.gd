...
var poor_score: int = 700
var good_score: int = 1200
var great_score: int = 1700
...
func _on_ready() -> void:
	...
	$Environment/EnemySweat.visible = false
	$Environment/EnemySour.visible = false
...
func _on_conductor_beat(position_of_song: Variant) -> void:
	...
	if song_position_in_beats > 0 and Global.killed_sweat:
		$Environment/EnemySweat.visible = true
	
	if song_position_in_beats > 0 and Global.killed_sour:
		$Environment/EnemySour.visible = true
	
	if song_position_in_beats > 0 and Global.entered_password and !call_once:
	...
	if song_position_in_beats > 4 and Global.finished_game and Global.entered_password:
	...
	if song_position_in_beats > 134:
		$Environment/EnemyDamage.visible = true
		damage_type = "Range"
		$Environment/EnemyDamage.frame = 4
...
func destroy_enemy():
	...
	if Global.killed_sour:
		var tween2: Tween = create_tween()
		tween2.tween_property($Environment/EnemySour, "modulate:v", 10, 1).from(1)
	elif Global.killed_sweat:
		var tween3: Tween = create_tween()
		tween3.tween_property($Environment/EnemySweat, "modulate:v", 10, 1).from(1)
...
func _on_end_timer_timeout() -> void:
	Global.finished_game = true
	Global.level_7 = false
	Global.killed_golem = true
	$Scoreboard.start()

func _on_dialogue_timer_timeout() -> void:
	$DialoguePlayer.dialogue_file = "res://Dialogue/level_7_end.json"
	$DialoguePlayer.start()
	get_tree().paused = true

func change_rewards():
	if Global.total_score > 10000:
		$Environment/Reward.frame = 6
		Global.big_prize = true
	elif Global.total_score > 3500:
		$Environment/Reward.frame = 7
		Global.medium_prize = true
	else:
		$Environment/Reward.frame = 8
		Global.small_prize = true

func show_reward():
	if !$Environment/Reward.visible:
		$Environment/Reward.visible = true
	else:
		$Environment/Reward.visible = false

func show_chest():
	if !$Environment/Chest.visible:
		$Environment/Chest.visible = true
	else:
		$Environment/Chest.visible = false

func show_paper():
	if !$Environment/Paper.visible:
		$Environment/Paper.visible = true
	else:
		$Environment/Paper.visible = false
